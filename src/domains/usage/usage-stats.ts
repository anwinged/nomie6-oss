...
export default {
  ...
  calculateSimpleMovingAverage(nums, window = 5, n = Infinity) {
    ...
    while (++indexinitial < window && numberOfSMAsCalculated++ < n) {
      const initialwindowSlice = nums.slice(indexinitial - indexinitial, indexinitial);
      const initialsum = initialwindowSlice.reduce((prev, curr) => prev + curr, 0);
  ...
  validateData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (Number.isNaN(data[i]) && !Number.isNaN(data[i + 1])) {
        ...
        if (validateddata[i] == undefined) {
          validateddata[i] = datamean;
        }
